...
//static uint8_t rom_buffer[1024*8];
static uint8_t *rom_buffer;
static uint32_t len=0;
static uint32_t Gamesize=0;
...
void readMd8(){
    ...
    while(adr<128){ // 64bytes/32word
	    ...
	    directRead16(); //save into read16 global
	    rom_buffer[adr] = (read16 >> 8)&0xFF; //word to byte
	    rom_buffer[(adr+1)] = read16 & 0xFF;
...
static void usbdev_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
   ...
   {
	    Gamesize = usb_buffer_IN[0] * 65536;
		dump_running = 1;
	}
...
rom_buffer = (unsigned char*)malloc(128);
...
dump_running = 0;
	...
	while(1)
	{
		...
		if(dump_running !=0 && len != Gamesize) // We are in dump mode so direct blast && len != Gamesize
			{
				gpio_set(GPIOC, GPIO13);
				GPIOA_BSRR |= TIME;
				 readMd8();			
				 usbd_ep_write_packet(usbd_dev, 0x82,rom_buffer,64);
				 address += 64;
			     len +=64;
				 //GPIOA_BRR |= TIME;
				gpio_clear(GPIOC, GPIO13);
